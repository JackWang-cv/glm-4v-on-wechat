...
from bot.zhipuai.zhipu_ai_vision import send
...
class ZHIPUAIBot(Bot, ZhipuAIImage):
    ...
    def reply(self, query, context=None):
        ...
        # 图生文 glm-4v
            ...
            # 传入图片http地址即可使用， 测试使用的
            # context.content
            # reply = Reply(ReplyType.TEXT, send("https://img.zcool.cn/community/0166d45de3e040a80120686b85c32c.jpg@1280w_1l_2o_100sh.jpg").content)
            # xxx为服务器图像的目录地址
            logger.info(f'create bot glm-4v for chat')
            reply = Reply(ReplyType.TEXT, send(context["text"], context.content).content)
            return reply
        else:
