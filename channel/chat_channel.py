...
class ChatChannel(Channel):
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        if first_in:  # context首次传入时，receiver是None，根据类型设置receiver
            ...
            if context.get("isgroup", False):
                ...
                context["receiver_nickname"] = cmsg.other_user_nickname
            else:
            ...
            if not e_context.is_pass() and reply and reply.type:
                ...
                if reply.type == ReplyType.TEXT:
                    ...
                    if context.get("isgroup", False):
                        ...
                        reply_text = "@" + context.get("receiver_nickname") + "\n" + reply_text + conf().get(
                            "group_chat_reply_suffix", "")
    ...
    def produce(self, context: Context):
        ...
        # 用于确保在多线程环境下对共享资源的访问是线程安全的。
        with self.lock:
            if session_id not in self.sessions:
                # 一个包含两个元素的列表。第一个元素是 Dequeue 对象，第二个元素是一个 threading.BoundedSemaphore 对象。
            ...
            with self.lock:
                ...
                for session_id in session_ids:
                    ...
                    if semaphore.acquire(blocking=False):  # 等线程处理完毕才能删除
                        # 如果 context_queue 不为空，它会从队列中获取一个 context 对象，并提交给 handler_pool 处理（即处理函数 _handle）。
                        if not context_queue.empty():
                            context = context_queue.get()
                            logger.debug("[chat_channel] consume context: {}".format(context))
                            # 大模型处理了 handler_pool.submit(self._handle, context) 会将 self._handle(context) 这个任务提交给线程池执行，
                            # 并返回一个 Future 对象。这个 Future 对象可以用来查询任务的执行状态、结果或异常。
                            # context是一个实例，包含询问的内容，生成的内容的类型
                            future: Future = handler_pool.submit(self._handle, context)
                            # 在任务完成后执行某些操作，比如处理任务结果或清理资源。
